...
def form_target_facts_list(values):
    facts = []
    for i, ell in enumerate(values):
        if ell:
            facts.append(f"#{ell}")
...
def form_clips_rules(df, facts_dict):
    ...
    for row in values:
        ...
        premises = [
            facts_dict[row[j]] for j in range(1, len(row)) if row[j] in facts_dict
        ]
        if premises:
            rules.append(f"{','.join(premises)}={conclusion};")
    return rules
...
def form_facts_and_rules_clips(df):
    ...
    target_facts = form_target_facts_list(unic_val)
